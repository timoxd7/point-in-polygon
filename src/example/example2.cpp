...
geo::Point<int> thepoint(width / 2, height / 2);
std::vector<geo::Point<int>> points;// a list of points of the polygon in order.
bool drawn = false;//flag indicating if all drawing are done in order to start detection
geo::Polygon<int>* polygon = NULL;
std::vector<geo::Polygon<int>*> polygons;//supporting drawing multiple polygons

void draw_polygon(geo::Polygon<int>& polygon) {
    glBegin(GL_LINE_STRIP);
    for (unsigned int i = 0; i < polygon.getCount(); ++i) {
        const geo::Point<int>& point = polygon.points[i];
...
void display(void)
{
    ...
    for (auto &polygon : polygons) {
        if (drawn && polygon->checkPointInPolygon(thepoint)) {
...
void keyboard(unsigned char key, int x, int y) {
    ...
    case 'D':
        ...
        if (!drawn) {
            ...
            if (points.size() != 0) {
                points.push_back(geo::Point<int>(points[0]));
            }
            polygons.push_back(new geo::Polygon<int>(points.data(), points.size()));
            points.clear();
        }
        break;
    case 'c':
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        if (!drawn) {
            points.push_back(geo::Point<int>(x, height - y));
        }
    }
}
